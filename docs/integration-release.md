...
本轮修改未连接生产数据库、改变 Secret 或执行部署；应用代码已提交 Git，生产发布仍需按受控 GitOps 流程完成。
...
头歌主流程和朔日服务适配、对话入口、结果归一化及契约测试已完成；朔日仍默认关闭。生产地址、隔离账号、依赖锁定、计费/退款规则和成绩提交授权尚未提供，不能直接开启朔日真实扣款或成绩提交。现有绑定仍是每客户一条；同时接入多个平台前需单独设计 `(customer_id, service)` 绑定迁移和任务调度隔离，不在本轮贸然修改生产主键。
